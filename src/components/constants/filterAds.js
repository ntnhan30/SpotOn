...
class FilterAds {
	...
	async init(ads, filterAtts, valueToFilter, key) {
		...
		for (var filterKey in tempFilt) {
			...
			if (tempFilt[filterKey].length > 0 && !stopFitering) {
				...
				for (let att in tempFilt[filterKey]) {
					...
					let ff = this.ads.filter(i =>
						filterList(i, filterKey, tempFilt[filterKey][att])
					);
				...
				if (
					...
					(keyFilteredAds && keyFilteredAds.length > 0)
				) {
					filteredAds = _.intersectionBy(
						keyFilteredAds,
						filteredAds,
						"adname"
					);
					...
					filteredAds = !(keyFilteredAds.length > 0)
						? this.ads
						: keyFilteredAds;
				}
			}
		...
		const result = [this.filterAtts, this.ads];
