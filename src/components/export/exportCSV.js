...
class ExportCSV extends Component{
    ...
    async componentDidMount() {
        let allResults = await this.props.functionsResults.getPercentileScore(this.props.toExport);
        this.setState({
    ...
    getOrderedValuesForCSV (typeOfReport) {
        const weightedValues = _.map( this.state.allResults, typeOfReport);
        const weighted = {};
    ...
    render() {
        ...
        const headerCSV = () => {
            return (
                <Workbook.Column key={0}  label='KPI' value='kpi' />
            )
        }

        // Creates the columns for the Excel export
        const columns = selectedAds.map(function(s, i) {
            return <Workbook.Column key={i+1} label={s.ad.adname} value={row => Math.round(row[s.ad.adname])}/>
        });

        const percentilColumns = selectedAds.map(function(s, i) {
            return <Workbook.Column key={i+1} label={s.ad.adname} value={row => (Math.round(row[s.ad.adname])+'th')}/>
        });
        // ----
        ...
        if (_.isEmpty(this.state.allResults)){
            ...
            return (
                <Fragment>
                    <Workbook filename="SpotOnReport.xlsx" element={<button className="btn"><span className="icon-in"></span>Download as Excel</button>}>
                    <Workbook.Sheet data={weightedValues} name="Weighted Report">
                        { columns }
                    </Workbook.Sheet>
                    <Workbook.Sheet data={percentileValues} name="Percentile Report">
                        { percentilColumns }
                    </Workbook.Sheet>
                    </Workbook>
                </Fragment>
